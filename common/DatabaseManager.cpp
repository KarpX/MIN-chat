...
void DatabaseManager::createTables() {
    sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, s_id INTEGER, r_id INTEGER, data BLOB, time TEXT, is_file INTEGER DEFAULT 0, file_name TEXT DEFAULT '', file_size INTEGER DEFAULT 0);", 0, 0, 0);
    sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS contacts (id INTEGER PRIMARY KEY, name TEXT);", 0, 0, 0);

    sqlite3_exec(db, "ALTER TABLE messages ADD COLUMN is_file INTEGER DEFAULT 0;", 0, 0, 0);
    sqlite3_exec(db, "ALTER TABLE messages ADD COLUMN file_name TEXT DEFAULT '';", 0, 0, 0);
    sqlite3_exec(db, "ALTER TABLE messages ADD COLUMN file_size INTEGER DEFAULT 0;", 0, 0, 0);
}
...
QVector<ContactRecord> DatabaseManager::getContactsWithLastMsg(int myId) {
    ...
    // Добавляем m.is_file и m.file_name в SELECT
    const char* sql =
        "SELECT latest.peer_id, COALESCE(c.name, 'User ' || latest.peer_id), m.data, (m.s_id = ?) as is_me, m.is_file, m.file_name "
        "FROM ("
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &st, 0) == SQLITE_OK) {
        ...
        while (sqlite3_step(st) == SQLITE_ROW) {
            ...
            // Читаем параметры файла для последнего сообщения
            rec.lastIsFile = sqlite3_column_int(st, 4) > 0;
            const char* fName = (const char*)sqlite3_column_text(st, 5);
            rec.lastFileName = fName ? QString::fromUtf8(fName) : "";
...
void DatabaseManager::saveMsg(int sid, int rid, const QByteArray& d, const QString& t, bool isFile, const QString& fileName, int fileSize) {
    sqlite3_stmt* st;
    sqlite3_prepare_v2(db, "INSERT INTO messages (s_id, r_id, data, time, is_file, file_name, file_size) VALUES (?, ?, ?, ?, ?, ?, ?);", -1, &st, 0);
...
QVector<MessageRecord> DatabaseManager::getMsgs(int myId, int targetId) {
    ...
    sqlite3_prepare_v2(db, "SELECT s_id, r_id, data, time, is_file, file_name, file_size FROM messages WHERE (s_id=? AND r_id=?) OR (s_id=? AND r_id=?) ORDER BY id ASC;", -1, &st, 0);
    sqlite3_bind_int(st, 1, myId);
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
        ...
        rec.isFile = sqlite3_column_int(st, 4) > 0;
        const char* fName = (const char*)sqlite3_column_text(st, 5);
        rec.fileName = fName ? QString::fromUtf8(fName) : "";
        rec.fileSize = sqlite3_column_int(st, 6);
